...
class LTRGBTTrain(TrainExperiment):
    MODEL_TYPE = 'ltr_gbt'
    TARGET_COL = 'score'
    RANDOM_SEED = 10
    N_JOBS = 5
    ...
    def pipeline(self) -> Pipeline:
        ...
        if self.cv:
            ...
            rf = GradientBoostingRegressor(random_state=self.RANDOM_SEED)
            rf = GridSearchCV(estimator=rf, param_grid=self.model_params, scoring=self.opt_metric, cv=self.cv,
                              n_jobs=self.N_JOBS)
        else:
            rf = GradientBoostingRegressor(random_state=self.RANDOM_SEED, **self.model_params)
        pipe = Pipeline(steps=[
